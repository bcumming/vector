...
TEST(device_coordinator, type_members) {
    using namespace memory;

    typedef memory::Storage<float,  16, 4> StorageFloatAoSoA;

    typedef device_coordinator<int> intcoord_t;
    typedef device_coordinator<StorageFloatAoSoA> storagecoord_t;
...
TEST(device_coordinator, rebind) {
    using namespace memory;
...
TEST(device_coordinator, baserange_alloc_free) {
    using namespace memory;
    ...
    EXPECT_NE(rng_t::pointer(0), rng.data()) << "device_coordinator returned a NULL pointer when alloating a nonzero range";
...
TEST(device_coordinator, refrange_alloc_free) {
    using namespace memory;

    typedef device_coordinator<float> floatcoord_t;
    ...
    EXPECT_NE(rrng_t::pointer(0), rrng.data())
        << "device_coordinator returned a NULL pointer when allocating a nonzero range";
...
// test that device_coordinator can correctly detect overlap between ranges
TEST(device_coordinator, overlap) {
    using namespace memory;
